 ...
 * @author Admin
 */
@WebServlet(name = "DiscussionPostServlet", urlPatterns = {"/discussionPost"})
public class DiscussionPostServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet DiscussionPostServlet</title>");
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet DiscussionPostServlet at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    }
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        if (status.equalsIgnoreCase("true")) {
            progressService.updateDiscussionCompletedProgress(userId, itemId);
        }
        if (discussionService.addDiscussionPost(userReply, userId, itemId)) {
...
//        for update post
        ...
        if (discussionService.updateDiscussionPost(postId, userReply)) {
            response.sendRedirect(request.getContextPath() + "/coursePage?itemId=" + itemId + "&courseId=" + courseId + "&pageNumber=" + pageNumber);
        }
    }
